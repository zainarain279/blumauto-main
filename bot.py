    ...
    while True:
        ...
        async with aiofiles.open(config_file) as r:
            ...
            config = Config(
                ...
                low=int(cfg.get("low", 1690)),
                high=int(cfg.get("high", 1750)),
...
{green}total proxy :{white} {len(proxies)}
    ...
    {green}7{white}.{green}) {white}start bot (sync mode)
        ...
        if opt == "3":
            ...
            continue
        if opt == "4":
            low = input(f"{green}input low game point : {white}") or 1690
            high = input(f"{green}input high game point : {white}") or 1750
            cfg["low"] = low
            cfg["high"] = high
            async with aiofiles.open(config_file, "w") as w:
                await w.write(json.dumps(cfg, indent=4))
